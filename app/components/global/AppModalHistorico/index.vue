<template>
  <Transition name="modal-fade">
    <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <div class="absolute inset-0 bg-black/60 backdrop-blur-sm transition-opacity" @click="fecharModal"></div>

      <div class="relative w-full max-w-4xl bg-white dark:bg-[#1e2029] rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] transform transition-all">
        ...
        <div class="flex-1 overflow-y-auto p-4 sm:p-6 scrollbar-custom bg-gray-50/50 dark:bg-transparent">
          ...
          <div v-else class="flex flex-col gap-3">
            ...
            >
              ...
              <Transition name="accordion-slide">
                <div v-show="itemAberto(index)" class="border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-[#1a1c23]">
...
const props = defineProps({
  ...
  historico: {
    type: Array as () => Array<{
      dataHora: string,
      usuario: string,
      alteracoes: Array<{ campo: string, valorAntigo: string, valorNovo: string }>
    }>,
    default: () => []
  }
})
...
// criei um array pra controlar quais itens do accordion estão abertos
const itensAbertos = ref<number[]>([])

const fecharModal = () => {
  emit('close')
  // adicionei um delay pra zerar o accordion só depois que a animação de fechar terminar
