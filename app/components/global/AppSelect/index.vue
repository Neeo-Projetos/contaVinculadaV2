  ...
  <div class="w-full relative">
    ...
    <div class="relative">
      <div @click="aberto = !aberto"
           class="w-full bg-gray-50 dark:bg-gray-900/50 rounded-xl px-4 py-3 text-sm text-gray-800 dark:text-gray-200 focus:outline-none transition-all cursor-pointer flex items-center justify-between shadow-sm border"
           :class="[aberto ? 'ring-2 ring-emerald-500/50 border-emerald-500' : 'border-gray-200 dark:border-gray-700/70', !modelValue ? 'text-gray-400 dark:text-gray-500' : '']">
        <span class="truncate pr-4">{{ textoSelecionado || placeholder }}</span>
        <Icon name="fa7-solid:chevron-down" class="w-3 h-3 text-gray-400 transition-transform duration-200 shrink-0" :class="{ 'rotate-180': aberto }" />
      </div>

      <div v-if="aberto" class="fixed inset-0 z-40" @click="aberto = false"></div>

      <Transition name="dropdown">
        <div v-if="aberto" class="absolute top-full mt-1.5 left-0 w-full z-50 bg-white dark:bg-[#1a1c23] border border-gray-200 dark:border-gray-700/80 rounded-xl shadow-2xl backdrop-blur-xl py-1 overflow-hidden">
          
          <div class="max-h-60 w-full overflow-y-auto overflow-x-hidden
