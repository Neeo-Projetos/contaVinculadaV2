...
export function useVerbaListagem() {
  ...
  const buscarSugestao = () => {
    ...
    timerDebounce = setTimeout(async () => {
      ...
      try {
        const resp = await $fetch<any>(`/api/tabelaBasica/verbas/autocomplete?q=${texto}`)
  ...
  const abrirHistorico = async (id: number) => {
    ...
    try {
      const response = await $fetch<any>('/api/tabelaBasica/verbas/historico', {
