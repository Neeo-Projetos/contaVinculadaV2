<template>
  <div class="min-h-full flex flex-col gap-4 p-4 md:p-6 animate-fade-in text-gray-900 dark:text-gray-100">
    ...
    <AppFiltro v-model="filtro" v-model:viewMode="visaoAtual" :campos="camposFiltro" titulo="Tipos de Movimentação"
      descricao="Gerenciamento de tipos de entrada e saída financeira" icone-titulo="fa7-solid:shuffle"
      :breadcrumbs="[{ label: 'Início', to: '/' }, { label: 'Tabela Básica' }, { label: 'Tipos de Movimentação' }]" :pending="carregando"
      @buscar="buscarLista" @openAdvancedFilter="() => {}"
    >
      <template #acoes>
        <AppBotao variacao="padrao" icone="fa7-solid:desktop" @click="() => {}">Controle de Exibição
        </AppBotao>
        <AppBotao variacao="acao" icone="fa7-solid:plus" @click="navigateTo('/tabelaBasica/tipoMovimentacao/cadastro?id=0')">
          Novo Tipo
        </AppBotao>
      </template>
      ...
      <AppContainerListagem ref="listagemRef" v-model:filtro-global="filtroGlobal" :carregando="carregando"
        ...
        :history="true" nomeTela="Tipo de Movimentação" endpointDelete="/api/tabelaBasica/tipoMovimentacao/excluir"
        @view="item => navigateTo(`/tabelaBasica/tipoMovimentacao/cadastro?codigo=${item.codigo}`)"
        ...
        <template #linhas-tabela="{ item }">
          <td class="px-6 py-4 text-center">
            <span class="text-xs font-bold text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded-lg">{{ item.codigo }}</span>
          </td>
          <td class="px-6 py-4">
            <NuxtLink :to="`/tabelaBasica/tipoMovimentacao/cadastro?codigo=${item.codigo}`" class="flex items-center gap-3 group">
              ...
              <div class="flex flex-col min-w-0">
                ...
                <span class="text-xs text-gray-500 dark:text-gray-400 truncate">Operação Financeira</span>
              </div>
            </NuxtLink>
          </td>
          <td class="px-6 py-4 text-center">
...
const camposFiltro = computed(() => [
  { key: 'descricao', label: 'Descrição', type: 'text' as const, placeholder: 'Buscar tipo...' },
  {
    key: 'ativo',
    label: 'Status',
