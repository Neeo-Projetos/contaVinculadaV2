      ...
      >
        ...
        <template #linhas-tabela="{ item }">
          <td v-if="colunas.descricao" class="px-6 py-4">
            <NuxtLink :to="`/tabelaBasica/tipoMovimentacao/cadastro?id=${item.codigo}&modo=visualizar`" class="flex items-center gap-3 group">
              <div
        ...
        <template #cards="{ item }">
          <AppCardListagem 
            ...
            @ver-detalhes="navigateTo(`/tabelaBasica/tipoMovimentacao/cadastro?id=${item.codigo}&modo=visualizar`)"
            @editar="navigateTo(`/tabelaBasica/tipoMovimentacao/cadastro?id=${item.codigo}`)"
            @excluir="() => listagemRef?.triggerDelete(item.codigo)"
            @ver-historico="abrirHistorico(item.codigo)"
            @clique-titulo="navigateTo(`/tabelaBasica/tipoMovimentacao/cadastro?id=${item.codigo}&modo=visualizar`)" 
          />
        </template>
