<template>
  <div class="min-h-full flex flex-col gap-6 p-4 md:p-8 animate-fade-in text-gray-900 dark:text-gray-100">
    <AppCabecalhoPagina 
      ...
      tituloGrosso="Classificação"
      descricao="Gerencie as classificações de movimentação do sistema" 
    ...
    <AppBarraFerramentas v-model:visao-atual="visaoAtual">
      ...
      <template #acoes-principais>
        ...
        <AppBotao variacao="primario" icone="fa7-solid:magnifying-glass" @click="buscarClassificacao">
          Pesquisar
        </AppBotao>
      </template>
    </AppBarraFerramentas>
    ...
    >
      ...
      <template #linhas-tabela="{ item }">
        ...
        <td class="px-6 py-4 text-right">
          <NuxtLink :to="`/tabelaBasica/classificacao/cadastro?id=${item.codigo}`" class="p-2 text-gray-400 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 rounded-xl transition-all inline-block" title="Editar">
      ...
      <template #cards="{ item }">
        <AppCardListagem 
          ...
          :ativo="item.ativo === 1"
          @ver-detalhes="navigateTo(`/tabelaBasica/classificacao/cadastro?id=${item.codigo}`)"
          @clique-titulo="navigateTo(`/tabelaBasica/classificacao/cadastro?id=${item.codigo}`)"
        >
...
const abrirHistorico = async (id: number) => {
  try {
    const response = await $fetch<{ data: any[] }>('/api/tabelaBasica/classificacao/historico', {
      method: 'POST',
