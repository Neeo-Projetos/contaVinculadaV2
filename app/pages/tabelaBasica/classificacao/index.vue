  ...
  <div class="min-h-full flex flex-col gap-4 p-4 md:p-6 animate-fade-in text-gray-900 dark:text-gray-100">
    ...
    >
      ...
      <AppContainerListagem ref="listagemRef" v-model:filtro-global="filtroGlobal" :carregando="carregando"
        ...
        :history="true" nomeTela="Classificação" endpointDelete="/api/tabelaBasica/classificacao/excluir"
        @view="item => navigateTo(`/tabelaBasica/classificacao/cadastro?codigo=${item.codigo}`)"
        ...
        <template #linhas-tabela="{ item }">
          <td class="px-6 py-4 text-center">
            <span class="text-xs font-bold text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded-lg">{{ item.codigo_classificacao }}</span>
          </td>
          <td class="px-6 py-4">
            <NuxtLink :to="`/tabelaBasica/classificacao/cadastro?codigo=${item.codigo}`" class="flex items-center gap-3 group">
              ...
              <div class="flex flex-col min-w-0">
                ...
                <span class="text-xs text-gray-500 dark:text-gray-400 truncate">Categoria de Lançamento</span>
              </div>
            </NuxtLink>
          </td>
          <td class="px-6 py-4 text-center">
...
const camposFiltro = computed(() => [
  { key: 'descricao', label: 'Descrição', type: 'text' as const, placeholder: 'Buscar classificação...' },
  {
    key: 'ativo',
    label: 'Status',
