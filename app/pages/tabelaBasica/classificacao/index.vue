          ...
          <td v-if="colunas.descricao" class="px-6 py-4">
            <NuxtLink :to="`/tabelaBasica/classificacao/cadastro?id=${item.codigo || item.id}&modo=visualizar`" class="flex items-center gap-3 group">
              ...
              <div class="flex flex-col min-w-0">
                ...
                <span class="text-xs text-gray-500 dark:text-gray-400 truncate font-medium">Categoria de Lançamento</span>
              </div>
            </NuxtLink>
          </td>
...
const camposFiltro = computed(() => [
  { 
      ...
      sugestoes: sugestoesClassificacao.value,
      buscando: buscandoClassificacaoSugestao.value,
      mostrarMenu: mostrarMenuClassificacao.value,
      icon: 'fa7-solid:tag'
  },
  {
    key: 'ativo',
