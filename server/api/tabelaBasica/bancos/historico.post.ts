...
export default defineEventHandler(async (event) => {
  ...
  const codigoBanco = Number(body.codigo)
  ...
  try {
    ...
    request.input('codigoBanco', codigoBanco)

    // Consulta o histórico ordenado pela data mais recente
    const result = await request.query(`
      SELECT 
        H.codigo, H.codigoBanco, H.nomeBanco, H.ativo, H.dataAlteracao,
        U.login AS usuarioAlteracao
      FROM tabelaBasica.bancoHistorico H
      LEFT JOIN configuracao.usuario U ON U.codigo = H.usuarioAlteracao
      WHERE H.banco = @codigoBanco
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (anterior) {
          ...
          if (valorAtual !== valorAnterior) {
            ...
            if (key === 'ativo') {
              itemHistorico.alteracoes.push({
                tipo: 'status',
                mensagem: valorAtual ? '- O banco foi ativado.' : '- O banco foi desativado.'
              })
            } else {
              itemHistorico.alteracoes.push({
                tipo: 'campo',
        ...
        itemHistorico.alteracoes.push({ tipo: 'criacao', mensagem: '- Banco cadastrado no sistema.' })
      }
